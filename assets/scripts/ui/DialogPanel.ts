...
export class DialogPanel extends Component {
  ...
  build(parent: Node) {
    ...
    this.panelRoot = createNode('DialogPaper', this.node, 610, 224, 0, 312);
    drawRect(this.panelRoot, 610, 224, hexToColor('#fff3d2', 248));
    applySlicedSprite(this.panelRoot, spritePaths.panelLight);
    const topRule = createNode('DialogTopRule', this.panelRoot, 520, 4, 0, 76);
    drawRect(topRule, 520, 4, hexToColor('#cda45a', 160));
    this.titleLabel = createLabel('RoundTitle', this.panelRoot, '', 540, 34, 20, hexToColor('#9b6c31'), 0, 90);
    this.narrativeLabel = createLabel('Narrative', this.panelRoot, '', 542, 88, 21, hexToColor('#2d2119'), 0, 18);
    this.hintRoot = createNode('SituationHint', this.panelRoot, 540, 40, 0, -72);
